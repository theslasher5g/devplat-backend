...
export class AgentClient {
  ...
  async createVm(teamId: string, ttlMinutes: number, vcpu: number, ramMb: number): Promise<AgentVm> {
    // Must clear the agent's own handler timeout (60s — see devplat-agent's
    ...
    const res = await agentFetch<{ vm_id: string; docker_endpoint: string }>(
      ...
      { method: 'POST', body: { team_id: teamId, ttl_minutes: ttlMinutes, vcpu, ram_mb: ramMb }, timeoutMs: 75_000 },
    );
